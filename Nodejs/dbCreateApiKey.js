...
//  if(result.rows[0].count)
	...
	{
		try{
			let query=`select count(client_name) from public.api_key_data where client_name='${origin}';`
			let result=await pool.query(query)
			console.log("count ",result.rows[0].count)
			if(result.rows[0].count>0)
			{
				query=`update public.api_key_data set client_public_key='${CLIENT_API_KEY}',client_secret_key='${CLIENT_SECRET_KEY}' where client_origin='${origin}';`
			...
			else
			{
				...
				query=`insert into public.api_key_data (client_public_key,client_secret_key,client_origin,client_name) values ('${CLIENT_API_KEY}','${CLIENT_SECRET_KEY}','${origin}','${origin}');`
				result= await pool.query(query);
				console.log(result)
...
async function GetAllApi()
{
	try{
		...
		let query=`select client_public_key,client_name,client_origin from public.api_key_data`;
